...
export function Card({ card, onClick, type = "row" }: Props) {
  const { user, setIsLoginDialogOpen } = use(UserContext);
  const { ownedCards, setOwnedCards } = use(CollectionContext);
  let amountOwned = useMemo(
    () =>
      ownedCards.find((row) => row.card_id === card.card_id)?.amount_owned || 0,
    [ownedCards]
  );
    ...
    async (cardId: string, newAmount: number) => {
      ...
      _inputDebounce[cardId] = window.setTimeout(async () => {
        ...
        if (ownedCard) {
          ...
          const newCard = await db.createDocument(
            DATABASE_ID,
            COLLECTION_ID,
            ID.unique(),
            {
  ...
  const Card_Row = () => {
    return (
      <div className="grid grid-cols-3 gap-x-3 w-full items-center">
        <h1 className="col-span-1">{card.name}</h1>
        <h2 className="col-span-1 text-center">{card.rarity}</h2>
        <div className="flex items-center gap-x-1 col-span-1 justify-end">
          <Button
            variant="ghost"
            size="icon"
  ...
  return type === "card" ? <Card_Fancy /> : <Card_Row />;
}
