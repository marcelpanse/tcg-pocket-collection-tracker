...
function CardDetail() {
  ...
  const row = useMemo(() => ownedCards.find((oc: CollectionRow) => oc.internal_id === card?.internal_id), [cardId])
  const alternatives = useMemo(
    () =>
      card?.alternate_versions.map((card_id) => ({ card_id, amount_owned: ownedCards.find((c) => c.internal_id === card?.internal_id)?.amount_owned ?? 0 })),
    [card, ownedCards],
  )
