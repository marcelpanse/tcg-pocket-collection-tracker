...
- Database schema: there is **no migration tooling**. All SQL lives in `scripts/sql/` and is applied
  by hand in the Supabase SQL editor:
  - `schema.sql` — a hand-committed `pg_dump` snapshot. Re-dump it with `pnpm schema:dump` after any schema
    change; it prompts for the DB password. Dumped with `--no-privileges`, so the column-level grants on
    `friends` are **not** captured — see below.
    Uses port 5432 on `db.<ref>.supabase.co`. Not 6543 (PgBouncer moved to `*.pooler.supabase.com` and 6543
    is now refused on the direct host) and not `supabase db dump` (runs `pg_dump` in a Docker container,
    which has no IPv6, while the direct host is IPv6-only).
  - `output/bulk-insert.sql` — generated by `pnpm scraper`; `TRUNCATE` + `INSERT` on `cards_list`.
  - `migrations/*.sql` — one-off data migrations, newest last.

#### `internal_id` encoding

`card_amounts` (the real collection table) and `decks.cards` are keyed by `internal_id`, a packed
integer built in `scripts/encoder.ts`: `(expansionId << 16) | (cardNr << 6) | rarityId`. Rarity is the
low 6 bits, which both Postgres (`internal_id & 63`) and the frontend rely on.

Consequence: **changing a card's rarity changes its `internal_id`.** Neither table has an FK to
`cards_list`, so stale rows do not error — they silently read as "not owned" and decks drop the card.
A post-release rarity fix therefore needs a remapping migration and a bump of the localStorage cache
key in `frontend/src/services/collection/collectionService.ts`. See
`scripts/sql/migrations/2026-08-b4a-rarity-remap.sql`.
